...
const QuotesGrid = ({ tags, quotesItems }) => {
  tags.sort((a, b) => a.name.localeCompare(b.name))
  quotesItems.sort((a, b) => a.title.localeCompare(b.title))
  ...
  function onClickTag(event) {
    ...
    if (filteredTags.indexOf(clickedTag) === -1) { // the clicked tag isn't already in the filter array
      ...
      // 2. filter the quotes items array based on the clicked tag, then put those items into the filteredItems array
      quotesItems.filter(quotesItem => quotesItem.tags.map(t => t.name).some(tag => [...filteredTags, clickedTag].includes(tag)))
  ...
  // A Quotes Item
  const Quotes = ({ quotesItem }) => (
    <li key={quotesItem.id} className="overflow-hidden col-span-1 flex flex-col rounded shadow bg-white hover:shadow-lg transition ease-in-out duration-150">
      <Link className="flex-1 flex flex-col" to={quotesItem.slug.current}>
        <Img className="w-full h-56 flex-shrink-0 mx-auto" fluid={quotesItem.image.asset.fluid} alt={quotesItem.title} />
        <h3 className="text-center mt-2 uppercase leading-5 font-semibold">{quotesItem.title}</h3>
        <p className="mb-2 text-center text-gray-700">{quotesItem.author.name}</p>
      </Link>
    </li >
  )
  ...
  return (
    <div className="max-w-screen-xl mx-auto min-h-screen ">
      ...
      <ul className="grid grid-cols-1 gap-12 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 pb-12">
        {filteredItems.length !== 0 && filteredItems.map((quotesItem, index) => <Quotes key={index} quotesItem={quotesItem} />)}
        {filteredItems.length === 0 && quotesItems.map((quotesItem, index) => <Quotes key={index} quotesItem={quotesItem} />)}
      </ul>
    </div>
  )
}
